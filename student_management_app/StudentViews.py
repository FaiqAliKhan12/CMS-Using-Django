...
def student_home(request):
    ...
    attendance_absent = AttendanceReport.objects.filter(studen_id = student_obj,status=False).count()
...
def student_view_attendance_post(request):
    if request.method == 'POST':
        ...
        user_obj = Students.objects.get(id=request.user.id)
...
def student_leave_apply_save(request):
    if request.method == 'POST':
        ...
        try:
            ...
            messages.error(request,"Failed to Leave")
            return redirect('student_apply_leave')
...
def student_feedback_save(request):
    if request.method != 'POST':
        ...
        try:
            ...
            add_feedback.save()
            messages.success(request,"Feedabck sent")
            return redirect('student_feedback')
        except:
